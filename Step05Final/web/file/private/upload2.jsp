...
<%
  ...
  /*
      위의 MultipartRequest 객체가 Exception 이 발생하지 않고 잘 생성이 되면 파일 업로드는 성공된 것이다.
      따라서 mr 에 들어있는 참조값을 이용해서 폼 전송된 값을 추출해서 DB에 저장만 잘 하면 된다.
  */
  //2. 폼 전송되는 title 을 읽어온다.
  String title = mr.getParameter("title");
  //3. 파일이 작성자(업로더)는 HttpSession 객체에서 읽어온다.
  String writer =(String)session.getAttribute("id");
  //4. 추가로 원본파일명, 저장된 파일명, 파일 사이즈도 얻어내서 FileDto 객체에 담아서
...
%>
{"isSuccess":<%=isSuccess%>, "savaFileName":"<%=saveFileName%>"}
<%--{"isSuccess":true, "saveFileName":"xxx.jpg"}--%>